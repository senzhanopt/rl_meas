...
path_saving = path_loading / 'result(baseline)'
...
def main(i_seed):
    ...
    soc_save = np.zeros((n_itr * quarter_num * day_num - 1, n_storage))
        # storage
    p_pd = pandas.read_excel('storage_p_1.xlsx', sheet_name='Sheet1', engine='openpyxl')
    q_pd = pandas.read_excel('storage_q_1.xlsx', sheet_name='Sheet1', engine='openpyxl')
    p_pd = p_pd.drop(columns='Unnamed: 0')
    q_pd = q_pd.drop(columns='Unnamed: 0')
    p_np = p_pd.to_numpy(dtype='float64')
    q_np = q_pd.to_numpy(dtype='float64')
